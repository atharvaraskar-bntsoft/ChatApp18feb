...
async function findAndDisplayConnectedUsers() {
    try {
        ...
        if (role === "MANAGER") {
            // Managers fetch only their assigned customers
            const response = await fetch(`/manager/${userId}/customers`);
            users = await response.json();
        } else if (role === "CUSTOMER") {
            // Customers fetch only their assigned manager
            const response = await fetch(`/customer/${userId}/manager`);
            const manager = await response.json();
            if (manager) users = [manager]; // Convert single object to array for display
        }
